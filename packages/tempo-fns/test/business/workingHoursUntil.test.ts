...
describe('workingHoursUntil', () => {
	...
	test('same day within working hours', () => {
		...
		expect(start.workingHoursUntil(end)).toBe(4.5);
	});
	...
	test('same day starting before working hours', () => {
		...
		expect(start.workingHoursUntil(end)).toBe(5.5);
	});
	...
	test('same day ending after working hours', () => {
		...
		expect(start.workingHoursUntil(end)).toBe(7);
	});
	...
	test('spanning over weekend', () => {
		...
		// Total = 5 hrs
		expect(start.workingHoursUntil(end)).toBe(5);
	});
	...
	test('spanning over holidays', () => {
		...
		// Total = 8 hrs
		expect(start.workingHoursUntil(end, { holidays: ['2026-12-25'] })).toBe(8);
	});
