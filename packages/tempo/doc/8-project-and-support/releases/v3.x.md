...
Introduced first-class support for historical dates. The new `{era}` formatting token automatically resolves to localized BC/AD or BCE/CE designations. Additionally, the core `Tempo` class now exposes `.era` and `.eraYear` zero-cost getters, providing immediate access to the underlying historical date components.
...
- **Ecosystem Alignment**: Updated `tempo.config.md` to remove deprecated plugin references, replacing them with accurate examples from the current `tempo-workspace` (`FinancePlugin` and `AstroTerm`).
