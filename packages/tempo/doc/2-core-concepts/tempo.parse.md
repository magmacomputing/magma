...
If your application processes custom log files or legacy data streams that position timezone designators at the *start* (e.g. `"PST 8 Aug 10:30"`), you can register a custom layout using `{tz}` alongside `{dt}` and `{tm}`. The `{tz}` token accepts both timezone offset designators (`+10:00`, `Z`, `GMT+10`) and registered timezone abbreviations (such as `AEST`, `PST`, `EST`):

```typescript
Tempo.init({
  registry: {
    layouts: {
      leadingTz: '{tz}{sep}+{dt}{sep}+{tm}'
    }
  }
});
