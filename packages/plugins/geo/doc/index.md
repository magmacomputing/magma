...
`Tempo.geo` is completely locked down:
- The `Tempo.geo` namespace object is frozen with `Object.freeze()`.
- The `geo` property on `TempoClass` is defined with `writable: false`, `configurable: false`, and `enumerable: false`.
