...
export class BoundedCache<K = string, V = string> extends Map<K, V> {
	 ...
	 */
	evictExpired(): void {
		if (this.#expires.size === 0) return;
		const now = Date.now();
		for (const [key, deadline] of this.#expires.entries()) {
			if (now > deadline) {
				this.delete(key);
			}
		}
	 ...
	 * @param ttl - Optional custom time-to-live in milliseconds for this specific entry
